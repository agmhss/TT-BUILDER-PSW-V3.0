...
// 🌟 NEW: Collaborative Editing Trackers
...
// 🌟 NEW: Collaborative Editing Logic
window.toggleEditMode = function() {
    window.isEditMode = !window.isEditMode;
    window.swapSource = null; // reset selection
    renderRegularTimetable(); // Re-render to show edit UI
    updateStatus(window.isEditMode ? "EDIT MODE: Click two cells to swap" : "GRID LOCKED");
};
...
window.handleCellClick = function(day, period) {
    ...
    if (!window.swapSource) {
        ...
        renderRegularTimetable(); // Re-render to highlight source
    } else {
        // Step 2: Select target and SWAP
        let target = { day, period };
        
        if (window.swapSource.day === target.day && window.swapSource.period === target.period) {
            // Clicked same cell, just deselect
            window.swapSource = null;
            renderRegularTimetable();
            return;
        }

        // Find the items in the generated data
        let viewType = document.getElementById('viewType')?.value;
        let filterVal = document.getElementById('viewFilter')?.value;

        // Since we are editing a filtered view, we need to know exactly WHICH teacher's or class's period we are moving.
        // We look for slots that match the current view and the clicked day/period.
        ...
        // Perform the swap in the array memory
        if (slot1Index !== -1) {
        ...
        window.swapSource = null; // Reset
        renderRegularTimetable(); // Re-render the grid
    }
};

// --- RENDER 1: REGULAR TIMETABLE (UPDATED FOR COLLABORATIVE EDITING) ---
function renderRegularTimetable() {
    ...
    // 🌟 NEW: Added the Edit Mode Toggle Button above the table
    let btnText = window.isEditMode ? "🔒 Lock Grid (Save Changes)" : "✏️ Enable Edit Mode";
    let btnClass = window.isEditMode ? "bg-red-100 text-red-700 border-red-300" : "bg-gray-100 text-gray-700 border-gray-300 hover:bg-gray-200";
    
    let html = `<div class="mb-4 flex justify-between items-center">
                    <h3 class="font-bold text-lg text-blue-900">${filterVal}</h3>
                    <button onclick="toggleEditMode()" class="px-4 py-1.5 border rounded-lg text-sm font-bold transition-all shadow-sm ${btnClass}">${btnText}</button>
                </div>
                <div class="overflow-x-auto"><table id="scheduleTable" class="w-full text-center border-collapse min-w-[800px] bg-white text-sm"><thead class="bg-blue-100 text-blue-900"><tr><th class="p-3 border border-blue-200 text-left w-24">Day</th>`;
    ...
    daysOfWeek.forEach(day => {
        ...
        teachingPeriods.forEach(period => {
            ...
            // 🌟 NEW: Collaborative Editing Styling
            let isSourceCell = window.swapSource && window.swapSource.day === day && window.swapSource.period === period.label;
            let cursorClass = window.isEditMode ? "cursor-pointer hover:bg-yellow-50 hover:ring-2 hover:ring-yellow-400 z-10 relative" : "";
            ...
            if (slot) {
                ...
                html += `<td ${clickEvent} class="p-2 border border-gray-200 transition-all align-middle leading-tight ${cursorClass} ${cellBgClass}">${cellText}</td>`;
            } else {
                let cellText = window.isEditMode && isSourceCell ? `<span class="text-xs font-bold text-yellow-800">Move here?</span>` : `-`;
                html += `<td ${clickEvent} class="p-2 border border-gray-200 ${cursorClass} ${cellBgClass}">${cellText}</td>`;
            }
        });
        html += `</tr>`;
    });
