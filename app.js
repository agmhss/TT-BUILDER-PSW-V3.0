 ...
 * Version: The Master Build V4.0 (10 Strict Rules Implemented)
 */
const APP_CONFIG = {
    fullName: "AGMHSS PSW", 
...
function getIndividualClasses(classNameStr) {
    ...
    groups.forEach(group => {
        ...
        if (parts.length < 2) {
            // Rule 8: Combined classes with '&'
            let combinedParts = group.trim().split(/[&]/);
            if (combinedParts.length > 1) {
                combinedParts.forEach(c => {
...
function generateAutoTimetable() {
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        // Rule 2 & 3: FN / AN Split Definitions
        function getMaxFnAn(req) {
            if (req.isClassTeacher) {
                // CT MUST get 5 FN periods (Period 1 x 5 Days). Remaining go to AN.
        ...
        // PHASE 1: CLASS TEACHERS MUST GET PERIOD 1 ON ALL 5 DAYS (Rule 6)
        ...
        // PHASE 2: CONTINUOUS LAST TWO PERIODS (Rule 4)
        // Subjects: SCI, PET, CE, CC
        currentAssignments.forEach(req => {
        ...
        // PHASE 3: CLUB IN LAST PERIOD (Rule 5)
        currentAssignments.forEach(req => {
            let sub = req.subjectName.toUpperCase();
            if (sub.includes('CLUB') || sub.includes('JRC') || sub.includes('NSS')) {
                let p = teachingPeriods[7]; // Period 8
                let daysToTry = [...daysOfWeek];
                if (iteration > 0) daysToTry.sort(() => Math.random() - 0.5);
        ...
        // Rule 1: Descending priority from Class 10 to Class 6
        currentAssignments.sort((a, b) => {
            ...
            // Monte Carlo with weighted grade priority
            ...
            for (let i = 0; i < req.periodsPerWeek; i++) {
                ...
                for (let offset = 0; offset < daysOfWeek.length; offset++) {
                    ...
                    for (let period of periodsToTry) {
                        // Rule 7: Non-Class Teacher should not be given the morning first period
                        if (!req.isClassTeacher && period.label === firstPeriod.label) continue; 

                        let isFN = fnPeriodLabels.includes(period.label);
                        
                        // Strict Balance Check (Bypassed for Part-Time teachers)
...
function renderExamSchedule() {
    ...
    activeGrades.forEach((grade, index) => {
        ...
        // RULE 10: DO NOT GIVE EXAM DUTY TO HM/AHM (marked with ⭐)
        let eligibleTeachers = presentTeachers.filter(t => !teacherProfiles[t].subjects.has("English") && !t.includes('⭐')); 
        if (eligibleTeachers.length === 0) eligibleTeachers = presentTeachers.filter(t => !t.includes('⭐')); 
        ...
        // If no eligible teachers found (e.g. everyone is HM/AHM), skip gracefully
...
function renderSubstituteSchedule() {
    ...
    vacantSlots.forEach(slot => {
        ...
        // RULE 10: HM/AHM (⭐) are excluded from Substitution Duty
