 ...
 * Version: Ultimate Build (Target Seeking 1000x + Strict CT Period 1 + Science Practicals + Cloud Memory)
 */
...
window.rawSavedGrid = null; // Cloud Grid Memory
...
// 🌟 PROTECT SAVED GRID FROM ACCIDENTAL "PROCESS" CLICKS
...
function generateAutoTimetable() {
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        // 1. CLASS TEACHERS MUST GET PERIOD 1 ON ALL 5 DAYS
        ...
        // 2. FIXED SPECIAL PERIODS FOR GRADES 6-10
        currentAssignments.forEach(req => {
            let gradeVal = getGradeValue(req.className);
            if (gradeVal >= 6 && gradeVal <= 10 && req.assignedCount < req.periodsPerWeek) {
                let sub = req.subjectName.toUpperCase();
                let targetDay = null;
                let targetPeriods = [];

                if (sub.includes('DRAW') || sub.includes('SEW') || sub.includes('ART')) {
                    targetDay = 'Monday'; targetPeriods = [teachingPeriods[6], teachingPeriods[7]]; 
                } else if (sub.includes('CLUB') || sub.includes('JRC') || sub.includes('NSS') || sub.includes('CO')) {
                    targetDay = 'Tuesday'; targetPeriods = [teachingPeriods[6], teachingPeriods[7]]; 
                } else if (sub.includes('MORAL') || sub.includes('VE') || sub.includes('VALUE')) {
                    targetDay = 'Friday'; targetPeriods = [teachingPeriods[7]]; 
                }

                if (targetDay) {
                    let indClasses = getIndividualClasses(req.className);
                    targetPeriods.forEach(p => {
                        if (p && req.assignedCount < req.periodsPerWeek) {
                            let timeKey = `${targetDay}-${p.label}`;
                            let isClassBusy = indClasses.some(cls => classAvail[cls]?.[timeKey]);
                            let isTeacherBusy = teacherAvail[req.teacherName]?.[timeKey];
                            let ptAvail = isPartTimeTeacherAvailable(req.teacherName, 'AN');

                            if (!isTeacherBusy && !isClassBusy && ptAvail) {
                                placeSlot(targetDay, p, req);
                            }
                        }
                    });
                }
        ...
        // 3. SCIENCE PRACTICALS (Double AN Priority)
        currentAssignments.forEach(req => {
            ...
            let needsDouble = isScience && isTargetGrade && periodsLeft >= 2 && req.assignedCount < 2;

            if (needsDouble) {
                let anPairs = [];
                for (let i = 4; i < teachingPeriods.length - 1; i++) {
                    anPairs.push([i, i+1]); 
                }
                let fnPairs = [];
                for (let i = 1; i < 3; i++) { 
                ...
                let tryPairs = (pairs) => {
                    ...
                    for (let day of daysToTry) {
                        ...
                        for (let pair of pairsToTry) {
                            ...
                            let ptAvail1 = isPartTimeTeacherAvailable(req.teacherName, fnPeriodLabels.includes(p1.label) ? 'FN' : 'AN');
                            let ptAvail2 = isPartTimeTeacherAvailable(req.teacherName, fnPeriodLabels.includes(p2.label) ? 'FN' : 'AN');
                ...
                if (!tryPairs(anPairs)) tryPairs(fnPairs); 
            }
        });

        // 4. MAIN PLACEMENT WITH STRICT FN/AN SPLIT
        currentAssignments.sort((a, b) => {
            if (iteration === 0) return b.periodsPerWeek - a.periodsPerWeek; 
            return Math.random() - 0.5; 
        });

        function attemptMainPlacement(req, allowLimitBypass) {
            let indClasses = getIndividualClasses(req.className);
            let maxDailyAllowed = Math.max(1, Math.ceil(req.periodsPerWeek / 5));
            let startDayIdx = iteration === 0 ? 0 : Math.floor(Math.random() * 5); 

            let key = `${req.className}-${req.subjectName}`;
            let allowedFnCount = Math.ceil(req.periodsPerWeek / 2);
            let allowedAnCount = Math.floor(req.periodsPerWeek / 2);
                ...
                for (let offset = 0; offset < daysOfWeek.length; offset++) {
                    ...
                    for (let period of periodsToTry) {
                        ...
                        if (!allowLimitBypass) {
                            ...
                            if (isFN && curFn >= allowedFnCount) continue;
                            if (!isFN && curAn >= allowedAnCount) continue;
                        }
        ...
        currentAssignments.forEach(req => attemptMainPlacement(req, false));
        currentAssignments.forEach(req => {
            if (req.assignedCount < req.periodsPerWeek) attemptMainPlacement(req, true);
        });
