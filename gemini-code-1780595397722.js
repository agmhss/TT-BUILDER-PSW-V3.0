 ...
 * Version: The Master Build (Deadlock Solver + PT Rules Fixed)
 */
...
function generateAutoTimetable() {
    ...
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        currentAssignments.forEach(req => {
            req.assignedCount = 0;
            req.hasDouble = false; // Flag for Science Practical
        });
        ...
        // 2. SCIENCE PRACTICALS (Double AN Priority)
        currentAssignments.forEach(req => {
            ...
            // Fixed: Check hasDouble flag instead of assignedCount < 2 so CT gets it too
            if (isScience && isTargetGrade && periodsLeft >= 2 && !req.hasDouble) {
                let anPairs = [];
                    ...
                    for (let day of daysToTry) {
                        ...
                        for (let pair of pairsToTry) {
                            ...
                            if (!isTeacherBusy && !isClassBusy && ptAvail1 && ptAvail2) {
                                ...
                                req.hasDouble = true; // Mark as successfully placed
                                return true;
                            }
                        }
                    }
        ...
        currentAssignments.sort((a, b) => {
            let loadA = window.teacherWorkload[a.teacherName] || 0;
            let loadB = window.teacherWorkload[b.teacherName] || 0;
            if (iteration === 0) return loadB - loadA; // Strongly prioritize busy teachers first
            return (loadB - loadA) * Math.random() + (Math.random() - 0.5) * 10;
        });
            ...
            for (let i = 0; i < req.periodsPerWeek; i++) {
                ...
                for (let offset = 0; offset < daysOfWeek.length; offset++) {
                    ...
                    for (let period of periodsToTry) {
                        ...
                        // Strict check - Bypassed for Part-Time teachers to prevent mathematical deadlocks
                        if (!allowLimitBypass && !isPT) {
...
window.runCalculationEngine = function(renderGrid = true, forceRecalculate = false) {
    try {
        ...
        // 🌟 UPDATED PT PARSING: Bulletproof recognition based on Master Sheet text
        for (let t in window.teacherMaxGrade) {
            window.teacherLevels[t] = getTeacherCategory(window.teacherMaxGrade[t]);
            let isMorn = tempTeacherSubjects[t]?.some(s => s.includes('MORNING') || s.includes('PT-FN'));
            let isAft = tempTeacherSubjects[t]?.some(s => s.includes('AFTERNOON') || s.includes('PT-AN'));
