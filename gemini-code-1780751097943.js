 ...
 * Version: The Ultimate Master Build V8.0 (Revamped Engine UI & UX)
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const headerDisplay = document.getElementById('loginSchoolName');
    if(headerDisplay) headerDisplay.innerText = APP_CONFIG.fullName;

    // Advanced Dropdown Listeners
                ...
                classSecStr.split(',').forEach(distinctClassGroup => {
                    ...
                    if (finalPeriods > 0) {
                        ...
                        SCHOOL_CONFIG.assignments.push({
                            teacherName: teacherName, subjectName: activity, className: cleanGroup, periodsPerWeek: finalPeriods, isClassTeacher: isCT
                        });
...
// ==========================================
// 🌟 THE CORE TIMETABLE ENGINE (Revamped)
// ==========================================
window.generateGrid = function() {
    ...
    // 🌟 Revamp: Show beautiful loading animation before freezing the UI
    mainGrid.innerHTML = `
    ...
    // Timeout allows the browser to render the loading screen before intense JS calculation
    setTimeout(() => {
        window.runCalculationEngine(false, true); // Calculate but don't render grid yet
        
        // Auto-Trigger the View by Class Dropdown
        const viewType = document.getElementById('viewType');
        if(viewType) {
            viewType.value = 'class';
            viewType.dispatchEvent(new Event('change')); // This automatically fires renderRegularTimetable
        }
    }, 100);
};
...
function generateAutoTimetable() {
    ...
    // 🌟 Revamp: Display Success Dashboard
    ...
    if (loadStatusDiv) {
        ...
        loadStatusDiv.innerHTML = `
            <div class="p-4 border-2 rounded-xl shadow-sm flex justify-between items-center ${color}">
                <div class="font-black flex items-center gap-2 text-lg">
                    <i data-lucide="${icon}" class="w-6 h-6"></i>
                    Engine Optimization Complete
                </div>
                <div class="font-bold text-sm bg-white px-4 py-2 rounded-lg shadow-sm border border-opacity-50 border-current">
                    Successfully Placed: <span class="text-xl font-black">${bestScore} / ${TARGET_SCORE}</span> Periods
                </div>
            </div>
        `;
...
window.addTeacherAllotmentUI = function() {
    ...
    window.uiAllotments.push({ teacherName: tName, ct: isCT ? tClass : "", act1: tSub, cls1: tClass, per1: tPeriods, act2: "", cls2: "", per2: "", act3: "", cls3: "", per3: "" });
    document.getElementById('newTSub').value = ''; document.getElementById('newTClass').value = '';
    renderUIAllotmentTable();
};
...
window.toggleRestSlot = function(cell) {
    ...
    if (!cell.classList.contains('selected') && currentlySelected >= 3) { alert("Max 3 rest periods allowed."); return; }
    cell.classList.toggle('selected');
    if (cell.classList.contains('selected')) {
        cell.classList.remove('bg-gray-50', 'hover:bg-red-50', 'hover:border-red-300');
        cell.querySelector('div').innerText = "REST"; cell.querySelector('div').classList.remove('text-transparent', 'group-hover:text-red-400');
    } else {
        cell.classList.add('bg-gray-50', 'hover:bg-red-50', 'hover:border-red-300');
        cell.querySelector('div').innerText = "Rest"; cell.querySelector('div').classList.add('text-transparent', 'group-hover:text-red-400');
    }
    updateRestCount();
};
...
window.submitTeacherConstraints = async function() {
    ...
    document.querySelectorAll('.rest-slot.selected').forEach(slot => {
        window.dynamicRules.push({ ruleId: 'T_' + currentUserName + '_' + Date.now(), targetType: 'Teacher', targetName: currentUserName, constraintType: 'Block Slot', parameter: slot.dataset.val, action: 'Hard' });
    });
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: "saveRules", data: window.dynamicRules }) });
